...
-   **Networking:** Client-server architecture. The `BlockBrawlers.Server.exe` will be a custom dedicated server executable managed and hosted by Azure PlayFab. Players connect as clients.
...
15. **SDK & Project Scaffolding:**
    ...
    * Create two new projects: `BlockBrawlers.Server` (a Console Application) and `BlockBrawlers.Shared` (a Class Library).
    * Move shared data structures (Vector3, PlayerData, SpellData) into the `BlockBrawlers.Shared` project.
...
18. **Prepare the Dedicated Server:** Integrate the PlayFab Game Server SDK into the `BlockBrawlers.Server` project. This allows the server executable to communicate its health and status to the PlayFab agent.
19. **Local Server Testing:**
    * Download and run the **`LocalMultiplayerAgent`** tool provided by Microsoft.
    * Launch your `BlockBrawlers.Server.exe` locally.
...
20. **Server Onboarding in PlayFab:**
    * Package the `BlockBrawlers.Server` build into a .zip file.
